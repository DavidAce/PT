...
class_hdf5_thermo_table::class_hdf5_thermo_table(std::nullptr_t nullp,
                                                 const std::string group_name_,
                                                 const std::string table_name_,
                                                 const bool mpi_on_ = false   )
        :
...
class_hdf5_thermo_table::class_hdf5_thermo_table(const std::string group_name_,
                                                 const std::string table_name_,
                                                 const bool mpi_on_ = false  )
        : class_hdf5_thermo_table(nullptr, group_name_,table_name_, mpi_on_)
{

}

void class_hdf5_thermo_table::initialize_table(){
...
void class_hdf5_thermo_table::write_buffer_to_file() {
    if (!this->empty() and table_is_ready) {
        hsize_t NRECORDS = this->size();
        hdf5_out->create_group_link(group_name);
